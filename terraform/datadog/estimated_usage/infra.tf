...
resource "datadog_monitor" "infra" {
  name               = "Datadog Infrastructure Host usage is high"
  type               = "metric alert"
  ...
  query = "sum(last_1h):datadog.estimated_usage.hosts{*} > ${var.datadog_log_ingested_commitment * ".8"}"
