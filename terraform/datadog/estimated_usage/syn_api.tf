...
resource "datadog_monitor" "syn_api" {
  name               = "Datadog Synthetic API Test usage is high"
  type               = "metric alert"
  ...
  query = "sum(last_1h):datadog.estimated_usage.synthetics.api_test_runs{*} > ${var.datadog_log_ingested_commitment * ".8"}"
