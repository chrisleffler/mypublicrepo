...
resource "datadog_monitor" "sessions" {
  name               = "Datadog RUM Session usage is high"
  type               = "metric alert"
  ...
  query = "sum(last_1h):datadog.estimated_usage.rum.sessions{*} > ${var.datadog_log_ingested_commitment * ".8"}"
